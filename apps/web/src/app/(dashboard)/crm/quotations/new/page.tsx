...
import { useCreateQuotation } from '@/hooks/use-crm'
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <section className="bg-white rounded-xl border border-gray-100 p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="date" className={inputClass} value={form.expiryDate} onChange={setField('expiryDate')} />
            </div>
          </div>
        </section>
              ...
              <tbody>
                {lines.map((line, i) => (
                  <tr key={i} className="border-b border-gray-50">
                    ...
                    <td className="py-2 text-right text-xs font-medium text-gray-800">{computedLines[i].total.toFixed(2)}</td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <section className="bg-white rounded-xl border border-gray-100 p-5 space-y-4">
            ...
            <div>
              ...
              <textarea rows={3} className={inputClass} value={form.notes} onChange={setField('notes')} placeholder="Thank you for considering us." />
            </div>
            <div>
              <label className={labelClass}>Terms & Conditions</label>
              <textarea rows={2} className={inputClass} value={form.terms} onChange={setField('terms')} />
            </div>
          </section>
        ...
        <div className="flex gap-3">
          ...
          <Link href="/crm/quotations" className="text-sm text-gray-500 hover:text-gray-700 px-4 py-2">Cancel</Link>
        </div>
      </form>
    </div>
  )
